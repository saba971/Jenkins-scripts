...
# this same script can be used for activation , download , commit, except post_ltb(in post_ltb script many addtionl info's are fetched(eg CS info)
...
#from the arguements got ,info from the job url is fetched.
...
#with the above console number ,details about the particular stage (downlod ,activation, etc) is fetched
burl="http://%s:8080/job/%s/%s/api/python" %(jenkins_ip,jname,console)
print burl
jsonResponse1 = eval(urllib.urlopen(burl).read())

#from below code build_id is fetched.Please consider the below code as an arrar format.Use the above url "burl" to understand the array format.
...
#from build id name , rerun data is fetched
if len(nbuild1) == 1:
	print "Not a rerun"
	jrerun="default"
else:
...
#below console url gives timestamp details.All these are based on jenkins format.So the code doesnt have to be canges unless official jenkins format is chenged 
...
#below code can be ignored once the MOSWA/SNMP,COPPER/FIBER,APME/ROBOT details are configured in PIPELINE_NAME variable in jenkins pipeline job.
...
if jbuild == abuild:
        try:
                cursor.execute("insert into %s VALUES('%s', '%s', '%s', '%s', '%s', '%s', '%s','%s','%s')" %(db_stage,jid, jname, jbuild, jstime, jetime, jstatus, jrerun,jetime-jstime,db_name))
                db.commit()
